...
def render_scrap_opportunity_analysis(inventory_data, currency="USD", scrap_threshold=730):
    ...
    with st.expander("📋 Detailed Scrap Candidate List", expanded=False):
        ...
        render_data_table(
            ...
            max_rows=100,
            downloadable=True,
            download_filename=f"scrap_candidates_{currency.lower()}.csv"
        )
...
def render_inventory_page(inventory_data):
    ...
    if not slow_movers.empty:
        ...
        render_data_table(
            ...
            max_rows=50,
            downloadable=True,
            download_filename=f"slow_moving_inventory_{currency.lower()}.csv"
        )
    else:
    ...
    with st.expander("📋 View All Inventory Records", expanded=False):
        ...
        render_data_table(
            ...
            max_rows=100,
            downloadable=True,
            download_filename=f"inventory_detail_{currency.lower()}.csv"
        )
